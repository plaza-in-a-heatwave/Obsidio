...
public class Warfrig extends Vessel {
	public static final String VESSELNAME = "warfrig";

    public Warfrig(GameContext context, String name, int x, int y) {
    ...
    public void setDefaultTexture() {
        this.setTexture(getVesselTexture("warfrig"));
        this.setOrientationPack(getContext().getTools().getGson().fromJson(
                Gdx.files.internal("assets/vessel/warfrig/sail.json").readString(),
                PackedObjectOrientation.class));
    }

    @Override
    public void setSinkingTexture() {
        this.setTexture(getVesselTexture("warfrig_sinking"));
        this.setOrientationPack(getContext().getTools().getGson().fromJson(
                Gdx.files.internal("assets/vessel/warfrig/sink.json").readString(),
                PackedObjectOrientation.class));
    }
}
